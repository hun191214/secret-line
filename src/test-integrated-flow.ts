...
async function main() {
  ...
  let reservationId = '';
  try {
    const reservation = await callService.reserveCallPayment('bob', 'joy');
  ...
  try {
    await callService.forceTerminateCall('bob', { emit: (...args: any[]) => console.log('[SOCKET]', ...args), disconnect: () => console.log('[SOCKET] disconnect()') }, 'agora-session-1');
