...
class ContentService {
    ...
    async getSecretVoiceList(userId, options = {}) {
    ...
    async getSecretVoice(contentId, userId) {
    ...
    async checkContentAccess(contentId, userId) {
        const content = await this.prisma.secretVoice.findUnique({ where: { id: contentId } });
        if (!content)
            return { hasAccess: false, isOwner: false, isPurchased: false, isFree: false };
        if (content.creatorId === userId)
            return { hasAccess: true, isOwner: true, isPurchased: false, isFree: false };
        if (content.price === 0)
            return { hasAccess: true, isOwner: false, isPurchased: false, isFree: true };
    ...
    async purchaseContent(contentId, userId) {
        try {
            const content = await this.prisma.secretVoice.findUnique({ where: { id: contentId } });
            if (!content || !content.isActive)
                throw new Error('구매 불가능한 콘텐츠입니다.');
            const existing = await this.prisma.contentPurchase.findFirst({ where: { contentId, userId } });
            if (existing)
                throw new Error('이미 구매한 콘텐츠입니다.');
            if (content.creatorId === userId)
                throw new Error('자신의 콘텐츠는 구매할 수 없습니다.');
            // 밀리골드 단위로 환산
