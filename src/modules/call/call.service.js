...
exports.callService = exports.getIdleCounselors = exports.CallService = void 0;
...
class CallService {
    ...
    // 통화 요금 (milli-Gold 단위)
    CALL_RATE_PER_SECOND = 25; // 1초당 25 milli-Gold
    MIN_RESERVE_SECONDS = 60; // 최소 1분 예약
    MIN_RESERVE_AMOUNT;
    constructor() {
    ...
    generateAgoraToken(params) {
        ...
        const token = agora_access_token_1.RtcTokenBuilder.buildTokenWithUid(this.AGORA_APP_ID, this.AGORA_APP_CERTIFICATE, channelName, uid, agoraRole, privilegeExpireTime);
        return token;
    }
    async validateCallAccess(userId) {
        try {
    ...
    async reserveCallPayment(userId, counselorId) {
        try {
    ...
    // 실시간 잔액 감시 및 알림/강제종료
    startBalanceMonitor(userId, socket, agoraSessionId) {
        // 1초마다 잔액 체크
    ...
    async forceTerminateCall(userId, socket, agoraSessionId) {
        try {
    ...
    // 10초 대기 시뮬레이션 (기초 구조)
    async waitForCounselor(counselorId, ms) {
        return new Promise((resolve) => setTimeout(() => resolve(false), ms));
    }
    // Failover 로직 (매출 방어)
    triggerFailover(userId) {
        try {
...
const getIdleCounselors = async () => {
    // Redis 연결 상태 체크 및 자동 연결
    if (redis_1.default && typeof redis_1.default.isOpen === 'boolean' && !redis_1.default.isOpen) {
        await redis_1.default.connect();
    }
    const keys = await redis_1.default?.keys?.('counselor:status:*') || [];
    const idleCounselors = [];
    for (const key of keys) {
        if (redis_1.default && typeof redis_1.default.isOpen === 'boolean' && !redis_1.default.isOpen) {
            await redis_1.default.connect();
        }
        const status = await redis_1.default?.get?.(key);
        const counselorId = key.split(':')[2];
        if (status === 'IDLE' && counselorId)
            idleCounselors.push(counselorId);
    }
    return idleCounselors;
};
exports.getIdleCounselors = getIdleCounselors;
