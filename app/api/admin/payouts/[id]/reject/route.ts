...
export async function PATCH(request: NextRequest, { params }: { params: RouteParams }) {
  try {
    ...
    await prisma.withdrawalRequest.update({
      where: { id },
      data: { status: 'REJECTED', processedAt: new Date(), processedBy: guard.user.id, rejectedReason: reason }
    });

    console.log(`❌ [정산 거절] 관리자 ${guard.user.email}: 요청 ID ${id} 거절 완료`);
    return NextResponse.json({ success: true, message: '정산 거절 완료' });
  } catch (error: any) {
